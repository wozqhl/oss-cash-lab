#!/usr/bin/env bash
# Dogfood A on E: generate TS/Python/Go clients from E otel-ai-cost OpenAPI.
# Output is regenerated (not committed) under bets/e-otel-ai-cost/sdk/generated/.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
A="$ROOT/bets/a-sdk-mcp-gen"
SPEC="${SPEC:-$ROOT/bets/e-otel-ai-cost/openapi/cost.openapi.json}"
OUT="${OUT:-$ROOT/bets/e-otel-ai-cost/sdk/generated}"
LANGS="${LANGS:-ts,python,go}"

if [ ! -f "$SPEC" ]; then
  echo "missing E OpenAPI: $SPEC" >&2
  exit 1
fi
...
if (!total) {
  console.error("E OpenAPI has zero path operations");
  process.exit(1);
}
if (missing.length) {
  console.error("E OpenAPI missing operationId:", missing.join(", "));
  process.exit(1);
}
...
echo "==> [A] generate from E OpenAPI -> $OUT (--lang $LANGS)"
...
# Assert tool/ops count > 0 and clients mention cost ops
node --input-type=module -e '
import fs from "node:fs";
...
for (const need of ["getHealth", "getIndex", "getReport", "getCostsCsv", "getCosts", "getOpenApi", "getMetrics"]) {
  if (!names.has(need)) {
...
for (const [label, src, needle] of [
  ["client.ts", ts, "getReport"],
  ["client.py", py, "getReport"],
  ["client.go", go, "GetReport"],
]) {
...
echo "dogfood-a-e OK — A clients from E OpenAPI (tools>0) at $OUT"
