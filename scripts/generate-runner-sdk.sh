#!/usr/bin/env bash
# Dogfood A on C: generate TS/Python/Go clients from C hosted-runner OpenAPI.
# Output is regenerated (not committed) under bets/c-agent-ci/sdk/generated/.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
A="$ROOT/bets/a-sdk-mcp-gen"
SPEC="${SPEC:-$ROOT/bets/c-agent-ci/openapi/runner.openapi.json}"
OUT="${OUT:-$ROOT/bets/c-agent-ci/sdk/generated}"
LANGS="${LANGS:-ts,python,go}"

if [ ! -f "$SPEC" ]; then
  echo "missing C OpenAPI: $SPEC" >&2
  exit 1
fi
...
if (!total) {
  console.error("C OpenAPI has zero path operations");
  process.exit(1);
}
if (missing.length) {
  console.error("C OpenAPI missing operationId:", missing.join(", "));
  process.exit(1);
}
...
echo "==> [A] generate from C OpenAPI -> $OUT (--lang $LANGS)"
...
# Assert tool/ops count > 0 and clients mention runner ops
node --input-type=module -e '
import fs from "node:fs";
...
for (const need of ["getHealth", "getReady", "getMetrics", "listRuns", "createRun", "getRun", "getRunJunit", "getRunTap", "listRunsTapTxt", "postCheckRuns", "listSuites", "getSuite"]) {
  if (!names.has(need)) {
...
for (const [label, src, needle] of [
  ["client.ts", ts, "createRun"],
  ["client.py", py, "createRun"],
  ["client.go", go, "CreateRun"],
]) {
...
echo "dogfood-a-c OK — A clients from C OpenAPI (tools>0) at $OUT"
