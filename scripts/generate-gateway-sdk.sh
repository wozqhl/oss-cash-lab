#!/usr/bin/env bash
# Dogfood A on B: generate TS/Python/Go clients from B gateway OpenAPI.
# Output is regenerated (not committed) under bets/b-mcp-gateway/sdk/generated/.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
A="$ROOT/bets/a-sdk-mcp-gen"
SPEC="${SPEC:-$ROOT/bets/b-mcp-gateway/openapi/gateway.openapi.json}"
OUT="${OUT:-$ROOT/bets/b-mcp-gateway/sdk/generated}"
LANGS="${LANGS:-ts,python,go}"

if [ ! -f "$SPEC" ]; then
  echo "missing B OpenAPI: $SPEC" >&2
  exit 1
fi
...
if (!total) {
  console.error("B OpenAPI has zero path operations");
  process.exit(1);
}
if (missing.length) {
  console.error("B OpenAPI missing operationId:", missing.join(", "));
  process.exit(1);
}
...
echo "==> [A] generate from B OpenAPI -> $OUT (--lang $LANGS)"
...
test -f "$OUT/mcp-server.mjs"
test -f "$OUT/mcp_server.py"
test -f "$OUT/mcp_server.go"

# Assert tool/ops count > 0 and clients mention gateway ops
node --input-type=module -e '
import fs from "node:fs";
...
for (const need of ["getHealth", "getReady", "toolsList", "toolsCall", "auditExport"]) {
  if (!names.has(need)) {
...
for (const [label, src, needle] of [
  ["client.ts", ts, "toolsCall"],
  ["client.py", py, "toolsCall"],
  ["client.go", go, "ToolsCall"],
]) {
...
echo "dogfood-a-b OK — A clients from B OpenAPI (tools>0) at $OUT"
