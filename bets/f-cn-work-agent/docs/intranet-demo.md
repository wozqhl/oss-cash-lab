...
> **Status:** local-mvp · ask/reply：`scripts/demo-ask-reply.sh` · 飞书/钉钉/企微审批（请假/用印）：`scripts/demo-feishu-approval.sh` / `demo-dingtalk-approval.sh` / `demo-wecom-approval.sh`  
...
| `approval_forward_secret` | 字符串 | 可选出站 HMAC（`APPROVAL_FORWARD_SECRET`；`X-Webhook-Signature: sha256=<hex>`；空 = 不签名，默认） |
