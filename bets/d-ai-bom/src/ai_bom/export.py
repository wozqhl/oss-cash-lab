...
SPDX 3 (`spdx3`) is a compact 3.0.1 JSON document from the same scan fields (software_File + contains only when a hashed file was observed; ai_AIPackage + profileConformance ai only when model path+sha256 or model-card fields were observed; not a full graph).
...
def _is_model_component(src: dict[str, Any]) -> bool:
    t = str(src.get("type") or "").strip().lower()
    return t in {"model", "model-file", "machine-learning-model"}


def _observed_ai_fields(src: dict[str, Any]) -> bool:
    """True when scan observed path+sha256 and/or model-card fields (never placeholders)."""
    if _observed_sha256(src):
        return True
    if src.get("cardName") not in (None, ""):
        return True
    if src.get("description") not in (None, ""):
        return True
    lic_url = str(src.get("licenseUrl") or "").strip()
    return lic_url.startswith(("http://", "https://"))
...
def to_spdx3(bom: dict[str, Any]) -> dict[str, Any]:
    ...
    ai_AIPackage + profileConformance ``ai`` only when a model/model-file
    component had observed path+sha256 and/or model-card fields.
    ...
    for i, src in enumerate(bom.get("components") or []):
        ...
        use_ai = _is_model_component(src) and _observed_ai_fields(src)
        pkg: dict[str, Any] = {
            "type": "ai_AIPackage" if use_ai else "software_Package",
            "spdxId": pkg_id,
            "name": cname,
            "creationInfo": creation_info,
            "software_downloadLocation": "NOASSERTION",
        ...
        if use_ai:
            has_ai_package = True
            pkg["software_primaryPurpose"] = "model"
        version = src.get("version")
        if version not in (None, ""):
    ...
    policy_hits = int(summary.get("policyHits") or 0)
    waived_n = len(summary.get("waived") or [])
    profiles = ["core", "software", "simpleLicensing"]
    if has_ai_package:
        profiles.append("ai")
    ai_note = (
        "ai_AIPackage + profileConformance ai only when a model/model-file had observed "
        "path+sha256 and/or model-card name/description/license URL. "
        if has_ai_package
        else "No SPDX 3 AI profile (no model path+sha256 or model-card fields observed). "
    )
    comment = (
        ...
        "software_Package / observed ai_AIPackage elements, simplelicensing_LicenseExpression, "
        ...
        "contains/dependsOn graph beyond observed files, trainedOn/testedOn datasets, "
        "AI metrics/hyperparameters/energy, security/CVE profile, "
