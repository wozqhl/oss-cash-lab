"""CORS allowlist: preflight OPTIONS + ACAO on GET. Default: disabled (no extra CORS).

Mirrors bets/e-otel-ai-cost/src/cors.js and bets/c-agent-ci/src/agent_ci/cors.py
(Python stdlib). Empty origins = deny extra CORS. '*' allows any Origin.
"""
...
DEFAULT_CORS_METHODS = ["GET", "HEAD", "OPTIONS"]
DEFAULT_CORS_HEADERS = ["Content-Type", "X-Request-Id"]
DEFAULT_CORS_EXPOSE_HEADERS: list[str] = ["Retry-After", "X-Request-Id"]

ENV_CORS_ORIGINS = "AI_BOM_CORS_ORIGINS"
...
def resolve_cors_origins(
    ...
    """CLI `--cors-origins` wins when provided (including empty); else env."""
