...
- [x] `clock --format md|html|ics` + evidence-pack `CLOCK.md` (same Markdown body as `--format md`; listed in pack.json `files` + MANIFEST + zip). Self-contained HTML (no CDN). RFC 5545 all-day VCALENDAR (stable UIDs; helper-not-certificate DESCRIPTION). Calendar/evidence helper, not a CRA compliance certificate. 日历/证据辅助，不是 CRA 合格证书。
