...
- [x] `clock` CLI (console script `ai-bom clock`; default as-of today UTC; optional `--as-of` / `--advisories`/`--dir`, `--format json|text`): days-until / days-overdue without an evidence zip. README 30s is `pip install -e .` then `ai-bom clock --format text`. Calendar/evidence helper, not a CRA compliance certificate. 日历/证据辅助，不是 CRA 合格证书。
