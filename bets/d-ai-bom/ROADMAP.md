...
- [x] `clock` CLI (console script `ai-bom clock`; default as-of today UTC; optional `--as-of` / `--advisories`/`--dir`, `--format json|text|gha|md|ics`): days-until / days-overdue without an evidence zip. README 30s is `pip install -e .` then `ai-bom clock --format text`. Calendar/evidence helper, not a CRA compliance certificate. 日历/证据辅助，不是 CRA 合格证书。
...
- [x] `clock --format md` bilingual Markdown for ticket paste (asOf + article14/sbom table + CLOCK_* disclaimer/notes; plain md, no HTML; exit 0). Calendar/evidence helper, not a CRA compliance certificate. 日历/证据辅助，不是 CRA 合格证书。
- [x] `clock --format ics` RFC 5545 VCALENDAR with two all-day VEVENTs (Article 14 2026-09-11 + SBOM 2027-12-11; stable UIDs; helper-not-certificate DESCRIPTION; exit 0). Calendar/evidence helper, not a CRA compliance certificate. 日历/证据辅助，不是 CRA 合格证书。
