#!/usr/bin/env python3
"""Tiny mock HTTP webhook receiver for D ai-bom local-mvp.
...
Records X-Webhook-Timestamp when present (OSS; replay window = paid).
  ...
  python3 mock-webhook-receiver.py --port 8816 --out out/webhook-last.json
  python3 mock-webhook-receiver.py --port 8817 --secret whsec_local_mvp \
...
from ai_bom.webhook import SIGNATURE_HEADER, TIMESTAMP_HEADER, verify_webhook_signature  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--out", default="out/webhook-last.json")
    parser.add_argument(
        "--headers-out",
        default=None,
