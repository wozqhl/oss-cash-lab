...
| Deps in generated clients | Stdlib only (no vendor SDK runtime). All 10 langs: 10s timeout, 429/5xx retry, per-op bearer/apiKey. iterate* page helpers on TS/Python/Go/Java/Kotlin/C# (not Rust; not a Stainless pager). User-Agent, per-attempt X-Request-Id, Idempotency-Key on writes (retries reuse the key). TS/Python/Go/Java/Kotlin/C# throw typed ApiError after retries (status, truncated body, sent X-Request-Id; 429 retry-after seconds when the header was present). MCP tools/call maps upstream failure to status + requestId (no auth headers). Rust/PHP/Swift/Ruby still surface raw HTTP failures. | Varies by generator; often a language HTTP stack. | Vendor-documented runtime (not measured here). | Vendor-documented (historical). | Vendor-documented. |
...
- Not a drop-in Stainless replacement. Stainless sold idiomatic, maintained SDKs. This emits stdlib clients plus MCP wiring — timeout/retry/auth/identity headers and typed ApiError on TS/Python/Go/Java/Kotlin/C#, not a Stainless-class SDK. No competitor error-handling scores or download counts.
